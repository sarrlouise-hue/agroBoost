...
class MaintenanceRepository {
	...
	async getStats() {
		// Coût par mois
		const costByMonth = await Maintenance.findAll({
			attributes: [
				[
					Maintenance.sequelize.fn(
						"date_trunc",
						"month",
						Maintenance.sequelize.col("startDate")
					),
					"month",
				],
				[
					Maintenance.sequelize.fn("SUM", Maintenance.sequelize.col("cost")),
					"totalCost",
				],
			],
			group: [
		...
		// Nombre par type de service
		// Utilisation de query raw pour simplifier le group by sur une table jointe
		const countByType = await Maintenance.sequelize.query(
			`SELECT "service"."serviceType", COUNT("Maintenance"."id") as "count"
       FROM "maintenances" AS "Maintenance"
       JOIN "services" AS "service" ON "Maintenance"."serviceId" = "service"."id"
       GROUP BY "service"."serviceType"`,
			{ type: Maintenance.sequelize.QueryTypes.SELECT }
		);
