...
if (databaseUrl) {
	...
	console.log(
		...
		urlForLog.split("@")[1] || "URL invalide"
	);
} else {
	console.error(
...
const useDirectOptions =
	process.env.NODE_ENV === "test" || !databaseUrl || DB.PASSWORD === undefined;
...
const connectDB = async () => {
	try {
		if (
			!databaseUrl ||
			databaseUrl === "postgresql://postgres:@127.0.0.1:5432/agroboost"
		) {
			console.error(
				"❌ DATABASE_URL non configurée ou utilise les valeurs par défaut."
			);
			console.error(
				"💡 Sur Vercel avec NeonDB, assurez-vous que la variable DATABASE_URL est définie dans les variables d'environnement Vercel."
			);
		...
		if (process.env.NODE_ENV === "development") {
			console.log("ℹ️  Synchronisation automatique désactivée (déjà fait).");
			/*
      ...
      */
        ...
        `);
				...
				const tablesExist =
					results &&
					results[0] &&
					(results[0].exists === true || results[0].exists === "t");
				...
				console.warn(
					"⚠️  Erreur lors de la vérification des tables:",
					syncError.message
				);
				...
				try {
					...
					if (
						...
						!createError.message.includes("déjà existe")
					) {
						throw createError;
					}
		...
		if (error.message.includes("ENOTFOUND")) {
			console.error(
				"💡 Vérifiez que la base de données NeonDB est provisionnée et que DATABASE_URL est correctement configurée sur Vercel."
			);
		}
		throw error;
	}
