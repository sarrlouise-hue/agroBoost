...
class ServiceService {
	...
	async createService(providerId, serviceData) {
		...
		if (!serviceData.pricePerHour && !serviceData.pricePerDay) {
			throw new AppError(
				"Au moins un prix (par heure ou par jour) doit être défini",
				400
			);
		}
	...
	async updateService(serviceId, providerId, updateData) {
		...
		if (
			filteredData.serviceType &&
			!Object.values(SERVICE_TYPES).includes(filteredData.serviceType)
		) {
			throw new AppError("Type de service invalide", 400);
		}
	...
	async getServicesByProvider(providerId, options = {}) {
		...
		const { count, rows } = await serviceRepository.findByProviderId(
			providerId,
			{
				page,
				limit,
