...
const createTransporter = () => {
	if (!isEmailConfigured()) {
		throw new AppError(
			"Configuration email incomplète. Veuillez configurer EMAIL_USER, EMAIL_PASSWORD et EMAIL_FROM_EMAIL dans votre fichier .env",
			500
		);
	}
...
const loadTemplate = async (templateName, variables = {}) => {
	try {
		...
		throw new AppError(
			`Impossible de charger le template ${templateName}`,
			500
		);
	}
};
...
const sendEmail = async (to, subject, html, text = null) => {
	try {
		...
		if (!isEmailConfigured()) {
			logger.warn(
				`⚠️  Email non configuré. Impossible d'envoyer l'email à ${to}. Configurez EMAIL_USER, EMAIL_PASSWORD et EMAIL_FROM_EMAIL dans votre .env`
			);
			...
			attachments: [
				{
					...
					cid: "logo", // doit correspondre à <img src="cid:logo"> dans le template
				},
			],
		};
		...
		if (
			error.message &&
			error.message.includes("Configuration email incomplète")
		) {
...
const sendPasswordResetRequestEmail = async (user, resetToken) => {
	const resetUrl = `${
		EMAIL.FRONTEND_URL || process.env.FRONTEND_URL || "http://localhost:3001"
