...
const generateToken = (userId, role) => {
	return jwt.sign({ userId, role }, JWT.SECRET, { expiresIn: JWT.EXPIRES_IN });
};
...
const generateRefreshToken = (userId) => {
	return jwt.sign({ userId, type: "refresh" }, JWT.REFRESH_SECRET, {
...
const register = async (userData) => {
	try {
		...
		const existingUserByPhone = await userRepository.findByPhoneNumber(
		...
		logger.info(
			`Utilisateur inscrit: ${user.phoneNumber}${
				user.email ? ` (${user.email})` : ""
			}`
		);

		// Ne PAS générer de tokens à l'inscription. L'utilisateur doit vérifier son email.
		...
		if (error.name === "SequelizeUniqueConstraintError") {
			const field =
				error.errors && error.errors[0] ? error.errors[0].path : "champ";
			if (field === "email") {
				throw new AppError("Un utilisateur existe déjà avec cet email", 409);
			}
		...
		if (error.code === 11000) {
			throw new AppError(
				"Un utilisateur existe déjà avec ces informations",
				409
			);
		}
...
const loginWithPassword = async (phoneNumber, password) => {
	try {
		...
		const user = await userRepository.findByPhoneNumberWithPassword(
			phoneNumber
		);
		...
		if (!user.password) {
			throw new AppError(
				"Aucun mot de passe défini. Utilisez la connexion par OTP.",
				400
			);
		}
...
const loginWithEmail = async (email) => {
	try {
		...
		// Note: On ne vérifie PAS isVerified ici car cette méthode est utilisée par verifyOTP
		// APRES avoir set isVerified = true.
		// Si on l'utilise pour le login magic link, il faudrait vérifier, mais pour l'instant
		// gardons-le permissif pour verifyOTP ou ajoutons un paramètre skipVerification.
		// Pour sécurité, verifyOTP devrait appeler une méthode qui génère juste les tokens.
		// Mais assumons que si verifyOTP l'appelle, user.isVerified est déjà true.
