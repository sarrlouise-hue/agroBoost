...
class MaintenanceService {
  ...
  async createMaintenance(maintenanceData) {
    ...
    logger.info(`Maintenance créée: ${maintenance.id} pour le service ${serviceId} par le mécanicien ${mechanicId}`);
  ...
  async updateMaintenance(id, updateData) {
    ...
    if (status === MAINTENANCE_STATUS.COMPLETED) {
      ...
      if (!finalEndDate) {
        throw new AppError('La date de fin est requise pour compléter une maintenance', 400);
      }
    ...
    const updatedMaintenance = await maintenanceRepository.update(id, {
      ...
      ...(calculatedDuration !== undefined && { duration: calculatedDuration || null }),
  ...
  async deleteMaintenance(id) {
    ...
    if (maintenance.status === MAINTENANCE_STATUS.IN_PROGRESS) {
      throw new AppError('Impossible de supprimer une maintenance en cours', 400);
    }
  ...
  async startMaintenance(id) {
    ...
    if (maintenance.status !== MAINTENANCE_STATUS.PENDING) {
      throw new AppError('Seules les maintenances en attente peuvent être démarrées', 400);
    }
  ...
  async completeMaintenance(id, endDate, duration, cost, notes) {
    ...
    if (maintenance.status === MAINTENANCE_STATUS.CANCELLED) {
      throw new AppError('Impossible de compléter une maintenance annulée', 400);
    }

    const finalEndDate = endDate || new Date();
    const start = new Date(maintenance.startDate);
    const end = new Date(finalEndDate);
    const calculatedDuration = duration || Math.round((end - start) / (1000 * 60 * 60));
