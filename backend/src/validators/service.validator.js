...
const createServiceSchema = Joi.object({
	serviceType: Joi.string()
		...
		.required()
		.messages({
			"any.only": `Le type de service doit être l'un des suivants: ${Object.values(
				SERVICE_TYPES
			).join(", ")}`,
	...
	model: Joi.string().optional().allow(null, "").messages({
	...
	year: Joi.number()
		...
		.optional()
		.messages({
			"number.min": "L'année doit être valide",
			"number.max": "L'année ne peut pas être dans le futur lointain",
		}),
	location: Joi.string().optional().allow(null, "").messages({
		"string.base": "La localisation doit être une chaîne de caractères",
	}),
	condition: Joi.string().optional().allow(null, "").messages({
...
const updateServiceSchema = Joi.object({
	...
	images: Joi.array().items(Joi.string()).single().optional().messages({
		"array.base": "Les images doivent être un tableau de chaînes",
	}),
