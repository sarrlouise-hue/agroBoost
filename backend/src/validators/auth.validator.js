...
const registerSchema = Joi.object({
	phoneNumber: Joi.string()
		...
		.messages({
			"string.pattern.base":
				"Le numéro de téléphone ne doit contenir que des chiffres et +",
	...
	password: Joi.string()
		.min(6)
		.required() // Password required for registration now
		.messages({
			"string.min": "Le mot de passe doit contenir au moins 6 caractères",
			"any.required": "Le mot de passe est requis",
		}),
	role: Joi.string()
		.valid("producteur", "prestataire", "user", "provider")
		.optional()
		.default("producteur")
		.messages({
			"any.only": "Le rôle doit être producteur ou prestataire",
		}),
	language: Joi.string().valid("fr", "wolof").optional().default("fr"),
});
