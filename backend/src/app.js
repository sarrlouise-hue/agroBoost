...
const swaggerOptions = {
	...
	customSiteTitle: "ALLO TRACTEUR API Documentation",
	swaggerOptions: {
 ...
 *                 message:
 *                   type: string
 *                   example: Bienvenue sur l'API ALLO TRACTEUR
 *                 version:
 *                   type: string
 *                   example: "1.0.0"
 *                 environment:
...
app.get("/", (req, res) => {
	res.status(200).json({
		success: true,
		message: "Bienvenue sur l'API ALLO TRACTEUR",
		version: "1.0.0",
...
const startServer = async () => {
	try {
		...
		if (!emailService.isEmailConfigured()) {
			...
			console.log(
				"   Les emails (OTP, bienvenue, réinitialisation) ne seront pas envoyés."
			);
			console.log(
				"   Pour activer l'envoi d'emails, configurez dans votre .env:"
			);
