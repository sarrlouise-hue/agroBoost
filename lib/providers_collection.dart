...
import 'package:allotracteur/models/api_response.dart';
import 'package:allotracteur/models/provider_model.dart';
...
class ProvidersCollection {
  ...
  Future<ApiResponse<Map<String, dynamic>>> getDashboardStats() async {
    final response = await _api.get('/providers/dashboard');
    return ApiResponse.fromJson(
      response,
  ...
  Future<ApiResponse<List<dynamic>>> getProviderBookings() async {
    final response = await _api.get('/providers/bookings');
    return ApiResponse.fromJson(
      response,
