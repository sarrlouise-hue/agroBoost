...
import 'package:allotracteur/pages/guest_tractor_detail_screen.dart';
import 'dart:math';
...
class _InteractiveMapScreenState extends State<InteractiveMapScreen> {
  ...
  Marker _createMarker(TractorData tractor, LatLng markerPoint) {
    return Marker(
      width: 40.0,
      height: 40.0,
      point: latlong2.LatLng(markerPoint.latitude, markerPoint.longitude),
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: const Icon(Icons.agriculture, color: Colors.white, size: 20.0),
        ),
      ),
    );
  }
  ...
  void _updateMarkers() {
    ...
    final Map<String, List<TractorData>> tractorsByLocation = {};
    for (var tractor in widget.tractors) {
      if (tractor.lat != 0.0 && tractor.lng != 0.0) {
        final locationKey =
            '${tractor.lat.toStringAsFixed(6)}_${tractor.lng.toStringAsFixed(6)}';
        if (!tractorsByLocation.containsKey(locationKey)) {
          tractorsByLocation[locationKey] = [];
        }
        tractorsByLocation[locationKey]?.add(tractor);
      }
    }
    final random = Random();
    for (var entry in tractorsByLocation.entries) {
      final tractorsAtLocation = entry.value;
      if (tractorsAtLocation.length == 1) {
        final tractor = tractorsAtLocation[0];
        final markerPoint = LatLng(tractor.lat, tractor.lng);
        newMarkers.add(_createMarker(tractor, markerPoint));
      } else {
        final baseLat = tractorsAtLocation[0].lat;
        final baseLng = tractorsAtLocation[0].lng;
        for (int i = 0; i < tractorsAtLocation.length; i++) {
          final double randomDistance = 0.0045 + random.nextDouble() * 0.009;
          final double randomAngle = random.nextDouble() * 2 * 3.14159265359;
          final offsetLat = baseLat + (randomDistance * cos(randomAngle));
          final offsetLng = baseLng + (randomDistance * sin(randomAngle));
          final markerPoint = LatLng(offsetLat, offsetLng);
          newMarkers.add(_createMarker(tractorsAtLocation[i], markerPoint));
        }
      }
    }
    if (mounted) {
