...
import 'package:allotracteur/services_collection.dart';
...
class _AddTractorScreenState extends State<AddTractorScreen> {
  ...
  Future<void> _saveTractor() async {
    ...
    try {
      ...
      print('name length: ${_nameController.text.length}');
      print('model length: ${_modelController.text.length}');
      print('type: ${_selectedType}');

      if (_selectedImage != null) {
        images = [_selectedImage!];
        final sizeKb = _selectedImage!.lengthSync() / 1024;
        debugPrint(
            '📏 TAILLE IMAGE (Fichier): ${sizeKb.toStringAsFixed(2)} KB');

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  '📤 Envoi du tracteur (${sizeKb.toStringAsFixed(2)} KB)...'),
        ...
        // Optionnel: Informer l'utilisateur qu'une image est recommandée
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  'Note: Ajouter une photo aidera les clients à vous trouver !'),
      ...
      // Map "Tracteur" to "tractor" for backend, others to lowercase
      final serviceTypeToSend = _selectedType == 'Tracteur'
          ? 'tractor'
          : (_selectedType?.toLowerCase() ?? 'labour');

      // Backend prohibits technicalSpecifications field, so we embed it in description
      final description =
          'Tracteur ${_modelController.text} - ${_yearController.text} - Heures: ${_engineHoursController.text}';
      ...
      print('🚀 SERVICE CREATION RESPONSE:');
      print('   Success: ${response.success}');
      print('   Message: ${response.message}');
      if (response.data != null) {
        print('   Data Name: ${response.data!.name}');
        print('   Data Images: ${response.data!.images}');
      }
      setState(() {
        _isLoading = false;
      });
      if (response.success && mounted) {
        // Show a confirmation dialog with image info
        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text('✅ Succès'),
            content: Text('Le tracteur "${response.data?.name}" a été ajouté.'),
            actions: [
              TextButton(
                onPressed: () {
