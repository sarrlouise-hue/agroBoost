...
import 'package:allotracteur/models/service_model.dart';
...
class _ReceivedReservationsScreenState
        ...
        .map(
          (booking) => {
            ...
            'clientName': booking.client != null
                ...
                : 'Client',
            'service':
                booking.service != null ? booking.service!.name : 'Service',
            'serviceObj': booking.service,
            'hectares': double.tryParse(_extractHectares(booking.notes)) ?? 0.0,
            'date': booking.bookingDate,
            'status': booking.status,
            'price': booking.totalPrice,
            'location': booking.client?.address ?? 'Localisation',
          },
        )
        .toList();
  }
                ...
                children: [
                  ClipRRect(
                    ...
                    child: SizedBox(
                      ...
                      height: 50.0,
                      child: SafeImage(
                        imageUrl: (booking['serviceObj'] != null &&
                                (booking['serviceObj'] as ServiceModel)
                                    .images
                                    .isNotEmpty)
                            ? (booking['serviceObj'] as ServiceModel).images[0]
                            : '',
                        fit: BoxFit.cover,
  ...
  Future<void> _loadReceivedBookings() async {
    try {
      ...
      final response = await ProvidersCollection.instance.getProviderBookings();
      if (mounted) {
        setState(() {
