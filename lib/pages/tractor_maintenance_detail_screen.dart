...
import 'package:hallo/pages/add_maintenance_screen.dart';
import 'package:hallo/pages/maintenance_report_screen.dart';
import 'package:hallo/pages/maintenance_planning_screen.dart';
import 'package:hallo/pages/edit_maintenance_screen.dart';
import 'package:hallo/components/maintenance_record_detail_sheet.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          if (tractor.imageUrl != null && tractor.imageUrl!.isNotEmpty)
            Container(
...
class _TractorMaintenanceDetailScreenState
  ...
  void _deleteMaintenance(String id) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text('✅ Entretien supprimé avec succès'),
  ...
  Future<void> _editMaintenance(MaintenanceRecord record) async {
    final result = await Navigator.push<MaintenanceRecord>(
      ...
      MaterialPageRoute(
        builder: (context) =>
            EditMaintenanceScreen(tractor: widget.tractor, record: record),
      ),
    );
    if (result != null) {
      setState(() {
        ...
        if (index != -1) {
          _maintenanceRecords[index] = result;
        }
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('✅ Entretien modifié avec succès'),
          backgroundColor: Theme.of(context).colorScheme.secondary,
          duration: const Duration(seconds: 2),
        ),
      );
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: const Color(0xff307033).withValues(alpha: 0.1),
                shape: BoxShape.circle,
              ),
              child: const Icon(
                Icons.calendar_month,
                size: 80.0,
                color: Color(0xff307033),
              ),
            ),
            const SizedBox(height: 24.0),
            Text(
            ...
            ElevatedButton.icon(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xff307033),
