...
import 'package:hallo/review_data.dart';
import 'package:hallo/pages/reservation_screen.dart';
...
class _TractorDetailScreenState extends State<TractorDetailScreen> {
  ...
  List<String> get _tractorImages {
    print('🖼️ TractorDetailScreen - Récupération des images du backend');
    print('   Image URL du tractor: ${widget.tractor.imageUrl}');
    if (widget.tractor.imageUrl.isNotEmpty) {
      print('   ✅ Image trouvée: ${widget.tractor.imageUrl}');
      return [widget.tractor.imageUrl];
    }
  ...
  Widget _buildSpecifications(bool isWolof) {
    // Extract engine hours from json safely
    String hours = '-';
    if (widget.tractor.technicalSpecifications.containsKey('engineHours')) {
      hours = widget.tractor.technicalSpecifications['engineHours'].toString();
    } else {
      // fallback if we stored it as 'hours' or something else, but we agreed on 'engineHours'
      hours = '-';
    }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSpecItem(
              Icons.branding_watermark, 'Marque', widget.tractor.brand),
          _buildSpecItem(Icons.build, 'Modèle', widget.tractor.model),
          _buildSpecItem(Icons.speed, 'Heures moteur', '$hours h'),
          _buildSpecItem(Icons.calendar_today, 'Année',
              widget.tractor.year != 0 ? widget.tractor.year.toString() : '-'),
          _buildSpecItem(
            Icons.agriculture,
            'Type',
            isWolof
                    ...
                    builder: (context) => AlertDialog(
                      ...
                      content: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16.0),
                          Text(
                            widget.tractor.providerPhone ?? '+221 77 xxx xx xx',
                      ...
                      actions: [
                        ...
                        ElevatedButton.icon(
                          onPressed: () {
                            Navigator.pop(context);
                            final phone =
                                widget.tractor.providerPhone ?? '+221770000000';
  ...
  Future<void> _makePhoneCall(String phoneNumber) async {
    final Uri launchUri = Uri(
      scheme: 'tel',
      path: phoneNumber.replaceAll(RegExp(r'\s+'), ''), // Remove spaces
    );
    try {
      if (await canLaunchUrl(launchUri)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content:
                    Text('Impossible de lancer l\'appel vers $phoneNumber')),
          );
        }
      }
    } catch (e) {
